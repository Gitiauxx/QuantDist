...
class GA(ABC):
    ...
    def __init__(self, pop_size=100, n_genes=2, mutation_rate=0.1, selection_rate=0.5, input_limits=[0, 1],
                 max_gen=100, stop=None, n_mutation=None):
    ...
    def process_muttion(self, population, ma, pa):
        raise NotImplementedError
    ...
    def run(self):
        ...
        while True:
            ...
            # random mutation:
            if self.n_mutation is not None:
                population = self.process_mutation(population, ma, pa)
            ...
            second_best_fitness = self.fitness_function(father)

            if gen_n % 1000 == 1:
                print(f'Iteration {gen_n}: Best fitness: {best_fitness}; Second best fitness: {second_best_fitness}')
